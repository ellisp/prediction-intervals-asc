...
competition <- function(collection, maxfors = length(collection)){
   ...
   mases <- foreach(i = 1:maxfors, .combine = "rbind") %dopar% {
      ...
      fc1 <- snaive(thedata$x, h = thedata$h)
      fc2 <- forecast(ets(thedata$x), h = thedata$h)
   ...
   colnames(mases) <- c("v", "e", "a", "n", "ve", "va", "vn", "ea", "en", "an",
                        "vea", "ven", "van", "ean", "vean")
   return(mases)
}
...
round(test1, 1)
sort(round(apply(test1, 2, mean), 1))
#========Fit models==============
...
leg <- "e: exponential smoothing\na: ARIMA
n: neural network\nv: naive or seasonally naive"

png("images/tcomp-ensembles.png", 6000, 3500, res = 600)
Tcomp_results %>%
...
png("images/mcomp-ensembles.png", 6000, 3500, res = 600)
Mcomp_results %>%
   ggplot(aes(x = model, y =  MASE, colour = Frequency, label = model)) +
   geom_text(size = 6) +
   geom_line(aes(x = as.numeric(model)), alpha = 0.25) +
